...
exports.run = async (client, message, args) => {
	...
	if(!findServer) return message.channel.send(
		"This server was not found in our list.\nAdd your server: https://paradisecode.gq/server/add"
	);
  	if (findUser) {
    ...
    await registerFont('src/fonts/font.ttf', { family: 'Paradise Development' })
    context.fillRect(0, 0, width, height)
    context.font = 'bold 60pt Paradise Development'
    ...
    .setStyle("url")
    .setURL("https://paradisecode.gq/server/"+message.guild.id)
...
function msgError(msg, { channel }) {
    ...
    .setFooter('paradisecode.gq/servers')
    .setDescription(msg)
    .setColor("RED")
    )
}